...
-- 民宿主多租户架构表结构
-- 版本: V1.0.7
-- 描述: 创建商户信息表、扩展sys_user表、添加角色数据
-- =============================================

-- 1. 扩展sys_user表，添加商户相关字段
ALTER TABLE sys_user
ADD COLUMN merchant_id BIGINT DEFAULT NULL COMMENT '商户ID（民宿主专用）' AFTER role_id,
ADD COLUMN merchant_status INT DEFAULT 0 COMMENT '商户状态（0待审核 1已认证 2已拒绝）' AFTER merchant_id,
ADD COLUMN apply_time DATETIME DEFAULT NULL COMMENT '申请时间' AFTER merchant_status;

-- 添加索引
ALTER TABLE sys_user ADD INDEX idx_merchant_id (merchant_id);
ALTER TABLE sys_user ADD INDEX idx_merchant_status (merchant_status);

-- 2. 创建商户信息表
CREATE TABLE IF NOT EXISTS merchant_info (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '商户ID',
    admin_user_id BIGINT NOT NULL COMMENT '管理员用户ID（关联sys_user.id）',
    merchant_name VARCHAR(100) NOT NULL COMMENT '民宿名称',
    merchant_code VARCHAR(50) NOT NULL COMMENT '商户编码（唯一）',
    ...
    property_proof VARCHAR(500) DEFAULT NULL COMMENT '房产证明（可多个，JSON格式）',
...
-- 6. 扩展room_info表，添加merchant_id字段
ALTER TABLE room_info
ADD COLUMN merchant_id BIGINT DEFAULT NULL COMMENT '商户ID' AFTER id;

-- 添加索引
ALTER TABLE room_info ADD INDEX idx_merchant_id (merchant_id);

-- 7. 扩展booking_order表，添加merchant_id字段
ALTER TABLE booking_order
ADD COLUMN merchant_id BIGINT DEFAULT NULL COMMENT '商户ID' AFTER id;

-- 添加索引
ALTER TABLE booking_order ADD INDEX idx_merchant_id (merchant_id);
